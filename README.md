...
And then use word2vec to select the best template having the higest similarity  between n * m keywords.
  ...
  * CFN-Vertex-Weight-TFIDF:
    * The same as CFN-Vertex-Weight, but use tfidf as value of connected component instead of frequency.
    * Selected Keywords:`墨爾本 愛華隆機場 交通方式 企鵝 天氣 交通工具 機場 墨爾本機場 `
    * Selected Template:`在查詢<汽車><性能>的資訊`
    * Output:`在查詢<交通工具><天氣>的資訊`
  * CFN-Vertex-Degree:
    * The same as CFN-Vertex-Weight, but use indegree as value of connected component instead of frequency.
    * Selected Keywords:`南半球 風是 冬季 交通 墨爾本機場 巴士 交通方式 愛華隆機場 `
    * Selected Template:`在查詢<汽車><性能>的資訊`
    * Template:`在查詢<巴士><天氣>的資訊`
  * CFN-PageRank:
    * The same as CFN-Vertex-Weight, but use pagerank score as value of connected component instead of frequency.
...
1. Generate Sequence with Accessibility logs：_`/behavior2text`_
  ...
  - method:`tfidf, kcem, CFN-Vertex-Weight, CFN-Vertex-Weight-TFIDF, CFN-Vertex-Degree, CFN-PageRank`
  ...
  - result:
    ```
    {'clusterTopn': '3', 'topN': '3', 'sentence': '在查詢季節X的資訊', 'method': 'CFN-Vertex-Weight'}
    ```

### Commands

1. `python3 manage.py experiment --topNMax=<(optional) at least 2> --clusterTopnMax=<(optional) at least 2> --percentageMax=<at least 2>`:
    * show NDCG of sequences when using each kind of methods: `tfidf, kcem, CFN-Vertex-Weight, CFN-Vertex-Weight-TFIDF, CFN-Vertex-Degree, CFN-PageRank`
